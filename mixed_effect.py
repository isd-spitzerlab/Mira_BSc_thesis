...
def build_niches_long(adata, radii, celltype_col, age_col, age):
    ...
    if not all_niches:
        raise RuntimeError("No EC niches were created. Check cell type labels, brain areas, age filter, and radii.")
...
def plot_ec_distance_histograms(
    ...
    fig, axes = plt.subplots(1, len(subtypes), figsize=(5 * len(subtypes), 4), sharex=True, sharey=True)
    if len(subtypes) == 1:
        axes = [axes]
    ...
    fig.savefig(os.path.join(output_dir, f"{safe_name(title_area)}_rel_dist_mural.pdf"), bbox_inches="tight", dpi=300)
    plt.close(fig)


def plot_ec_distance_boxplots(
    ec_df,
    output_dir,
    brain_area=None,
    subtypes=("aECs", "capECs", "vECs"),
...
def plot_ec_distance_boxplots_with_stats(
    sample_means_df,
    overall_df,
    pairwise_df,
    output_dir,
